...
const SERVICES = [
    ...
    service('saas-development', 'SaaS開発', '認証、課金、管理画面、顧客導線まで含め、本番運用を前提としたSaaSや新規プロダクトを開発します。', ['新規事業を検証したい', '月額課金サービスを作りたい', '本番品質の初期版を短期間で公開したい'], '80万円〜', '1〜3ヶ月'),
    ...
    service('mvp-development', '自社プロダクト運営知見を活かした初期開発', '自社SaaSやプロダクト運営経験を活かし、本番運用と継続改善を前提とした初期版を構築します。', ['初期版を作りたい', '作り直しを抑えて本番公開したい'], '25万円〜', '2〜6週間')
];
...
const BEST_FOR = [
    'AIを業務に導入したい中小企業',
    'SaaSやWebサービスを本番品質の初期版から始めたい事業者',
    '既存業務を自動化したい企業',
    'ChatGPTやClaude時代に対応したHP/LPを整備したい企業'
];

const CASE_STUDIES = [
    caseStudy('AI業務システム', '業務自動化', '問い合わせ対応や見積作成が属人化し、担当者ごとに対応品質がばらつく。', '問い合わせ内容の分類、見積作成補助、管理画面、メール通知を含む本番運用可能な構成で業務を標準化する。', ['問い合わせ分類', '見積作成補助', '管理画面', 'メール通知'], '対応漏れや手戻りを減らし、初期段階から業務改善効果を確認しやすくする。', ['AI開発', '業務自動化', '管理画面開発']),
    caseStudy('SaaS初期開発', 'SaaS開発', '新規サービスを作りたいが、作り直しによる二重コストは避けたい。', '認証、基本画面、管理機能、データ構造を含む本番運用可能な初期版を構築する。', ['認証', 'ユーザー画面', '管理画面', '利用状況確認'], '本番公開後の顧客反応を見ながら継続改善へ進める。', ['SaaS開発', 'Webサービス開発', '初期開発']),
...
const PATTERNS = {
    P1: pattern('本番接続型・初期リリース', '本番接続型初期開発プラン', '25万円〜60万円', '最短2〜4週間', ['中核機能', '問い合わせ入力フォーム', '管理画面', 'メール通知'], '中核機能を優先しながら、本番運用と継続改善ができる品質で公開する構成が適切です。'),
    P2: pattern('AI業務システム', '業務システム自動化プラン', '35万円〜150万円', '3〜6週間', ['問い合わせ入力フォーム', 'AI分類', '管理画面', 'メール通知', '診断結果保存'], '属人化している業務を洗い出し、問い合わせ分類、見積作成補助、管理画面を含む本番運用可能な構成で始めるのが適切です。'),
    P3: pattern('SaaS・新規プロダクト型', 'SaaS初期開発プラン', '80万円〜300万円', '1〜3ヶ月', ['認証', '管理画面', '課金導線', 'ユーザーダッシュボード'], '収益化を見据え、認証・管理・課金導線を含む本番品質の初期版を構築し、公開後に改善する構成が適切です。'),
    ...
    P6: pattern('要件整理・相談スタート型', '要件整理プラン', 'まずは無料相談から', '相談30分 → 最短2〜4週間', ['要件整理', '優先順位づけ', '画面構成ラフ', '概算見積'], 'まだ要件が固まっていない場合は、目的、利用者、優先機能と運用要件を整理してから開発するのが適切です。')
};
...
function runDiagnosis(input = {}) {
    ...
    const assumptions = [
        ...
        '作り捨てを前提にせず、本番運用に必要な基盤を含めて設計します。',
...
function generateProjectBrief(input = {}) {
    ...
    const goals = Array.isArray(input.goals) && input.goals.length ? input.goals.map((v) => clean(v, 160)).filter(Boolean) : ['現状課題を整理する', '本番運用に必要な初期範囲を決める', '概算見積と期間を確認する'];
    const scope = ['主要画面・主要機能の整理', '必要データと連携先の確認', '本番接続を見据えた検証機能の実装'];
    return {
        projectBrief: `相談概要: ${summary || '会話内容から開発相談を整理します。'}\n相談種別: ${clean(input.projectType || '未定', 120)}`,
        goals,
        scope,
        outOfScope: ['確定見積の提示', '契約条件の確定', 'ユーザー同意なしの問い合わせ送信'],
        assumptions: ['作り捨てを前提にせず、本番運用に必要な基盤を含めて設計します。', 'OpenAI API / Claude APIの自社利用は必須にしません。'],
